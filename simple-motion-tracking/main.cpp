...
void searchForMovement(Mat thresholdImage, Mat &cameraFeed) {
    // Referece, & operator is used to get continued feed from camera
    
    bool objectDetected = false;
    Mat temp;
    thresholdImage.copyTo(temp);

    // Following vectors give us the output of findContours
	...
	if (contours.size() > 0) {
        // Contours vector is not empty so we've found some objects
        objectDetected=true;
	} else {
...
int main(int, char**) {
	...
	while(1) {
		...
		capture.open("/Users/st/projects/OpenCV_Stuff/simple-motion-tracking/bouncingBall.avi");
		...
		while(capture.get(CAP_PROP_POS_FRAMES) < capture.get(CAP_PROP_FRAME_COUNT) - 1) {
			...
			switch(waitKey(10)) {
                ...
                case 100: // 'D' has been pressed, this will debug mode
                    ...
                    break;
                case 112: // 'P' has been pressed, this will pause or resume the code.
                    pause = !pause;
                    if(pause == true) { 
